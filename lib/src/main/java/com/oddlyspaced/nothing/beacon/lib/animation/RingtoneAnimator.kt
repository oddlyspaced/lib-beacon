...
class RingtoneAnimator(private val context: Context) {
    ...
    private fun animate() {
        CoroutineScope(Dispatchers.IO).launch {
            while (isRinging) {
                if (mediaPlayer.currentPosition < mediaPlayer.duration - 1) {
//                    Log.d("OKK", "${mediaPlayer.currentPosition} ${mediaPlayer.duration}")
